...
function dissipate(w: WorldState, s: Storm): void {
  // Under a remote authority, whether a storm dies is not this copy's call.
  // The server sends the death, and the debris ring with it.
  if (w.remoteAuthority) return;
  s.alive = false;
...
function stepStorm(w: WorldState, s: Storm, input: Input, dt: number): void {
  ...
  if (newRank >= RANK_EL_NINO && warmPoolT(s.x, s.y) <= 1) {
    s.winTimer += dt;
    if (s.winTimer >= WIN_HOLD_TIME && w.winner < 0 && !w.remoteAuthority) {
...
export function step(w: WorldState, inputs: Map<number, Input>): void {
  ...
  for (let i = 0; i < w.storms.length; i++) {
    ...
    if (!s.alive) {
      // A dead storm under a remote authority is one the server stopped
      // sending: out of view, not gone. Respawning it here would put a
      // phantom on screen.
      if (s.kind === 'bot' && !w.remoteAuthority) {
        s.botTimer -= dt;
        if (s.botTimer <= 0) {
          respawnBot(w, s);
          designateLaNina(w);
    ...
    if (!s.alive) {
      ...
      else if (s.id === w.playerId && !w.remoteAuthority) w.over = true;
    }
  }
