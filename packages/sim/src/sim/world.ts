...
export interface WorldOptions {
  ...
  /** The storms belong to a server; see WorldState.remoteAuthority. */
...
/** Dry land on all eight headings at `d` units out. */
function inland(w: WorldState, x: number, y: number, d: number): boolean {
  for (let k = 0; k < 8; k++) {
    const a = (k / 8) * Math.PI * 2;
    const n = sampleTerrain(
      w.terrain,
      wrapX(x + Math.cos(a) * d),
      clamp(y + Math.sin(a) * d, 0, WORLD_H),
    );
    if (n.water || n.ice) return false;
  }
  return true;
}

/**
 * Find a land spawn. Three passes, each looser than the last: good feeding
 * ground with room to move, then the same with half the elbow room, then any
 * land at all.
 *
 * A Dust Devil carries only 2 mass of headroom above the dissipation floor, so
 * spawning within a few seconds' flight of an ice cap or a coastline is a
 * two-second run. Start people where the food is, with somewhere to go.
 */
export function findLandSpawn(w: WorldState, tries = 220): [number, number] {
  const ICE_MARGIN = 700;
  // Elbow room, measured rather than guessed: cold water costs 1.8 mass a
  // second against 2 of opening headroom, so a spawn one turn from the surf
  // is a drowning the player had no way to avoid and reads as the game being
  // broken. 400 units is three quarters of a second of flight on the WORST of
  // eight headings, and about a quarter of the map's land clears it - enough
  // that the strict pass almost always succeeds. 200 is the fallback; below
  // that a coastal scrap beats no spawn at all.
  const COAST_MARGIN = 400;
  const COAST_MARGIN_RELAXED = 200;
  for (let i = 0; i < tries; i++) {
    ...
    // Dry land on all eight headings, so every opening move is survivable and
    // the first thirty seconds are about finding food rather than about which
    // way the coast happened to be.
    if (!inland(w, x, y, COAST_MARGIN)) continue;
    return [x, y];
  }

  // Same again with half the margin, before giving up on the idea entirely.
  for (let i = 0; i < tries; i++) {
    const x = rand(w) * WORLD_W;
    const y = randRange(w, WORLD_H * 0.3, WORLD_H * 0.7);
    const t = sampleTerrain(w.terrain, x, y);
    if (t.water || t.ice || t.biome === 5) continue;
    if (!inland(w, x, y, COAST_MARGIN_RELAXED)) continue;
    return [x, y];
  }
  for (let i = 0; i < tries; i++) {
    const x = rand(w) * WORLD_W;
...
export function createWorld(opts: WorldOptions): WorldState {
  ...
  const w: WorldState = {
    ...
    remoteAuthority: opts.remoteAuthority ?? false,
    debrisCursor: 0,
